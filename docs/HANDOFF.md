...
- **本番デプロイ完了**：[`https://nesh.kkweb.io`](https://nesh.kkweb.io) (Vercel alias: `nesh-red.vercel.app`)
  - Vercel scope: `kkweb`、プロジェクト: `nesh`、GitHub 連携済(自動デプロイ)
  - カスタムドメイン: `nesh.kkweb.io` (Cloudflare CNAME → `cname.vercel-dns.com`、proxy off、Let's Encrypt SSL)
