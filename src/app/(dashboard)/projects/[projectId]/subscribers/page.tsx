...
export default async function Page({ params, searchParams }: Props) {
  ...
  if (trimmed) {
    // Match against endpoint OR external_user_id (case-insensitive contains).
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">{project.name} · subscribers</h1>
        ...
        <CardContent>
          <form className="flex gap-2" action={`/projects/${project.id}/subscribers`}>
            <Input
              ...
              name="q"
              defaultValue={q}
              placeholder="Match user ID or endpoint…"
              className="max-w-md"
            />
            ...
            {trimmed ? (
              <Button asChild variant="ghost" size="sm">
                <Link href={`/projects/${project.id}/subscribers`}>Clear</Link>
              </Button>
            ) : null}
          </form>
        </CardContent>
        ...
        <CardContent className="p-0">
          {subs && subs.length > 0 ? (
            ...
            <p className="p-6 text-sm text-muted-foreground">
              {trimmed ? "No subscribers match that search." : "No subscribers yet."}
            </p>
          )}
        </CardContent>
      </Card>
