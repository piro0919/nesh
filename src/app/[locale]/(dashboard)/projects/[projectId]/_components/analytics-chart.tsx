...
export async function AnalyticsChart({ projectId }: Props) {
  const t = await getTranslations("dashboard.analyticsChart");
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("title", { days: DAYS })}</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat label={t("notifications")} value={totals.sent.toLocaleString(locale)} />
          <Stat label={t("delivered")} value={totals.delivered.toLocaleString(locale)} />
          <Stat
            label={t("shown")}
            value={totals.shown.toLocaleString(locale)}
            sub={totals.delivered > 0 ? t("ofDelivered", { rate: showRate }) : undefined}
          />
          <Stat
            label={t("clicked")}
            value={totals.clicked.toLocaleString(locale)}
            sub={totals.shown > 0 ? t("ofShown", { rate: clickRate }) : undefined}
          />
        </div>
        <div
