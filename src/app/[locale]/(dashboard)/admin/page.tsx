  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat label={t("stat.projects")} value={totalProjects ?? 0} />
        <Stat label={t("stat.subscribers")} value={totalSubs ?? 0} />
        <Stat label={t("stat.notificationsAllTime")} value={totalNotifications ?? 0} />
        <Stat label={t("stat.notificationsThisMonth")} value={monthlyNotifications ?? 0} />
      </div>
        ...
        <CardContent className="overflow-x-auto p-0">
          <table className="w-full text-sm">
            <thead className="bg-muted/30 text-xs text-muted-foreground">
              <tr>
                <th className="px-4 py-2 text-left">{t("breakdown.project")}</th>
                <th className="px-4 py-2 text-right">{t("breakdown.subs")}</th>
                <th className="px-4 py-2 text-right">{t("breakdown.sendsMo")}</th>
                <th className="px-4 py-2 text-right">{t("breakdown.shown")}</th>
                <th className="px-4 py-2 text-right">{t("breakdown.clicked")}</th>
                <th className="px-4 py-2 text-right">{t("breakdown.ctr")}</th>
                <th className="px-4 py-2 text-left">{t("breakdown.created")}</th>
              </tr>
            </thead>
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td className="px-4 py-2 text-right">{r.subscribers.toLocaleString(locale)}</td>
                    <td className="px-4 py-2 text-right">
                      {r.sends_this_month.toLocaleString(locale)}
                    </td>
                    <td className="px-4 py-2 text-right">{r.shown_total.toLocaleString(locale)}</td>
                    <td className="px-4 py-2 text-right">
                      {r.clicked_total.toLocaleString(locale)}
                    </td>
          ...
          {rows.length === 0 ? (
            <p className="p-4 text-sm text-muted-foreground">{t("breakdown.empty")}</p>
          ) : null}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {(errors?.length ?? 0) === 0 ? (
            <p className="text-sm text-muted-foreground">{t("errors.empty")}</p>
          ) : (
